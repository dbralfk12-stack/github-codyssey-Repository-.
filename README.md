...
**1. 서버를 '배(화물선)'에 비유해봅시다 (왜 도커를 쓰는가?)**
- **기존의 서버 (도커 이전):** 서버라는 배에 쌀(A프로그램), 생선(B프로그램), 옷(C프로그램)을 그냥 싣습니다. 그러다 생선 냄새가 옷에 배거나, 쌀이 젖는 등 서로 충돌(의존성/버전 문제)이 발생합니다.
- **가상 머신(VM) 방식:** 충돌을 막기 위해 배 안에 쌀 전용 작은 배, 생선 전용 작은 배를 따로따로 띄웁니다. 너무 무겁고 공간 낭비가 심합니다. (OS를 통째로 띄워야 함)
- **도커(Docker)의 해결책:** 모든 화물을 **규격화된 철제 컨테이너(Container)**에 깔끔하게 나누어 담습니다! 서로 섞이지 않고 격리되어 안전하며, 배(OS 커널)는 하나만 공유하므로 아주 가볍고 빠릅니다.

**2. 도커의 3가지 핵심 요소 (이미지, 컨테이너, 허브)**
- **Image (이미지):** 컨테이너를 완벽하게 똑같이 찍어낼 수 있는 **설계도**이자 **틀**입니다. 한 번 만들어지면 변하지 않습니다(불변성).
- **Container (컨테이너):** 틀(이미지)에서 찍어내어 배 위에 올라간 **실제 철제 박스**이자 **실행 중인 프로세스**입니다. 쓰고 나서 지우거나 새로 만들 수 있습니다.
- **Docker Hub (도커 허브):** 전 세계 사람들이 자신이 만든 틀(이미지)을 올려놓은 **거대한 앱 스토어**입니다. 여기서 다운로드(Pull) 받기만 하면 누구든 환경 설정 없이 1초 만에 실행할 수 있습니다.
