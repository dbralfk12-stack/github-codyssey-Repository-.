...
**💡 Git(깃)이란 무엇이며, 왜 사용해야 할까요?**
- **Git이란?** 파일의 변경 내역을 추적하고 여러 사람과 안전하게 협업할 수 있게 해주는 **'분산형 버전 관리 시스템'**입니다. 쉽게 말해 코드의 '강력한 타임머신'이자 '블랙박스'라고 할 수 있습니다.
- **왜 써야 할까? (Git의 3대 장점):**
  1. **완벽한 백업과 복구:** 코드를 잘못 수정해서 프로그램이 망가져도, 언제든지 정상 작동하던 과거의 특정 시점(커밋)으로 1초 만에 되돌릴 수 있습니다. 더 이상 `최종_진짜최종.txt` 같은 파일명 지옥을 겪지 않아도 됩니다.
  2. **안전한 병렬 협업 (브랜치):** 하나의 프로젝트를 여러 팀원이 동시에 수정해도 코드가 꼬이지 않습니다. 각자 자신만의 독립적인 작업 공간(브랜치)에서 안전하게 코드를 짠 뒤, 나중에 원본에 합칠(Merge) 수 있습니다.
  3. **투명한 히스토리 추적:** "누가, 언제, 왜, 어느 줄의 코드를" 수정했는지 모든 기록이 영원히 남습니다. 덕분에 버그의 원인을 찾거나 다른 사람에게 업무를 인수인계하기가 매우 수월해집니다.
